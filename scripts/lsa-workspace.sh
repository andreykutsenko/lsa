...
#
# Usage:
#   lsa-workspace.sh [TICKET] --snap <SNAP> [--cid CID] [--jobid JOBID] [--pick PICK]
#                             [--title "..."] [--mode snap|ssh] [--rhs-host rhs] [--rhs HOST]
#                             [--root DIR]
#
# Shortcuts:
#   --ssh-copy  (same as --mode ssh)
#   --rhs-copy  (same as --mode ssh)
...
Arguments:
  TICKET              Ticket identifier (e.g. INC0123456) — optional.
                      If omitted, a name is derived from --title or timestamp.
...
TICKET=""
# First positional arg (if not a flag) is treated as TICKET
if [[ $# -gt 0 && "$1" != --* && "$1" != -* ]]; then
  TICKET="$1"; shift
fi
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --ssh-copy|--rhs-copy) MODE="ssh"; shift ;;
...
# If no ticket provided, derive workspace name from title or timestamp
if [[ -z "$TICKET" ]]; then
  TICKET="${TITLE:-ws}_$(date +%Y%m%d_%H%M%S)"
fi
...
LOGFILE="$WS/logs/lsa-workspace.log"
log() {
...
#!/usr/bin/env bash
# Auto-generated by lsa-workspace.sh
...
NOTE="$WS/notes/${TICKET}.md"
{
  ...
  echo
  echo "## Logs"
  echo "- Script log: \`logs/lsa-workspace.log\`"
